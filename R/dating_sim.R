...
for (M in M_range){
  result <- rep(0, niter)
  for (i in 1:niter){
    order <- perm_rank(100) #simulate the order
...
plot(1:100, prob_result, main = 'Probability of finding best partner for N = 100 \n by exploring first M candidates',
     xlab = 'M values', ylab = 'Probability')

theo <- sapply(1:100, function(x) theo_prob(x))
lines(1:100, theo, col = 'red')
legend("bottomright", legend=c("Simulated","Theoretical"), col=c("black","red"), lty=c(NA,1),lwd=c(NA,2),pch=c(1,NA))
